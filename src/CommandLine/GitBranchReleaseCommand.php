...
class GitBranchReleaseCommand extends GitBranchCommand
{
    ...
    public function run()
    {
        ...
        $this->type = Git::BRANCH_TYPE_RELEASE;

        if ($this->branch) {
            return $this->branch();
        } elseif (Input::confirm('Are you sure you want to merge the release and close the branch?')) {
            $this->commit();
            $this->close();
